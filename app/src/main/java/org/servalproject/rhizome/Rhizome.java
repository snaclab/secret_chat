	...
	public static void registerMessageHandlers(ServalDMonitor monitor){
		ServalDMonitor.Messages handler = new ServalDMonitor.Messages() {
            ...
            if(manifest instanceof RhizomeManifest_MeshMS){
              ...
              if (identity!=null && identity.sid.equals(meshms.getRecipient()))
                if (ServalBatPhoneApplication.context.meshMS!=null){
                  Intent intentGroupMessageService = new Intent(ServalBatPhoneApplication.context, GroupMessageService.class);
                  intentGroupMessageService.putExtra("sender", meshms.getSender().toString());
                  app.startService(intentGroupMessageService);
            }
            }
